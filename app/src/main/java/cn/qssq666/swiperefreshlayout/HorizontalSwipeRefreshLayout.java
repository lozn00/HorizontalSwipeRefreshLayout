 ...
 * contents of a view via a Horizontal swipe gesture. The activity that
...
//        mCurrentTargetOffsetTop = mCircleView.getTop();
    ...
    public void setProgressViewOffset(boolean scale, int start, int end) {
        mScale = scale;
        mCircleView.setVisibility(View.GONE);
        mOriginalOffsetLeft = mCurrentTargetOffsetLeft = start;
...
//        mCircleView.layout((height / 2 - circleHeight / 2), mCurrentTargetOffsetLeft,
//                (height / 2 + circleHeight / 2), mCurrentTargetOffsetLeft + circleWidth);
...
//                (width / 2 + circleWidth / 2), mCurrentTargetOffsetTop + circleHeight);
    ...
    public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (!mUsingCustomStart && !mOriginalOffsetCalculated) {
            ...
            mCurrentTargetOffsetLeft = mOriginalOffsetLeft = -mCircleView.getMeasuredWidth();
//            mCurrentTargetOffsetLeft = mOriginalOffsetLeft = -mCircleView.getMeasuredHeight();
...
//                return ViewCompat.canScrollVertically(mTarget, -1) || mTarget.getScrollX() > 0;
            }
        } else {
...
//                final float y = getMotionEventY(ev, mActivePointerId);
                ...
                final float yDiff = x - mInitialDownX;
...
/*
    ...
    public void onNestedPreScroll(View target, int dx, int dy, int[] consumed) {
        ...
        // before allowing the list to scroll DY 修改为dx
...
//                : mSpinnerFinalOffset;
        ...
        int targetY = mOriginalOffsetLeft + (int) ((slingshotDist * dragPercent) + extraMove);
//        int targetY = mOriginalOffsetTop + (int) ((slingshotDist * dragPercent) + extraMove);
        ...
        setTargetOffsetLeftAndRight(targetY - mCurrentTargetOffsetLeft, true /* requires update */);
...
//            animateOffsetToStartPosition(mCurrentTargetOffsetTop, listener);
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        if (!isEnabled() || mReturningToStart || canChildScrollLeft() || mNestedScrollInProgress) {
            // Fail fast if we're not in a state where a swipe is possible
            return false;
        }
/*
...
//                final float overscrollTop = (y - mInitialMotionY) * DRAG_RATE;
    ...
    private void animateOffsetToStartPosition(int from, Animation.AnimationListener listener) {
        if (mScale) {
            ...
            startScaleRightReturnToStartAnimation(from, listener);
        } else {
            mFrom = from;
...
//        setTargetOffsetTopAndBottom(offset, false /* requires update */);
    ...
    private void startScaleRightReturnToStartAnimation(int from,
                                                       Animation.AnimationListener listener) {
        mFrom = from;
        if (isAlphaUsedForScale()) {
            mStartingScale = mProgress.getAlpha();
            ...
            mStartingScale = ViewCompat.getScaleX(mCircleView);
//            mStartingScale = ViewCompat.getScaleY(mCircleView);
        }
        mScaleDownToStartAnimation = new Animation() {
            @Override
            public void applyTransformation(float interpolatedTime, Transformation t) {
